    ...
    return (
        <li className="overflow-hidden">
            ...
            <div className="flex justify-between mb-2 ">
                <p className="font-medium truncate">{make}{' '}<span className="text-blueText">{model}</span>,{' '}{year}</p>
            ...
            <div className="text-xs text-greyText whitespace-nowrap mb-7">
                <div className="flex mb-1">
                    ...
                    <p className="truncate">{rentalCompany}</p>
                </div>
                <div className="flex">
                    <p className={classAfter}>{type}</p>
                    <p className={classAfter}>{make}</p>
                    <p className={classAfter}>{mileage}</p>
                    <p className="truncate">{functionalities[0]}</p>
                </div>
            </div>
            <LearnMoreBtn setIsOpenModal={setIsOpenModal} />
            {isOpenModal && <CarInfo car={car} />}
        </li>
    )
}